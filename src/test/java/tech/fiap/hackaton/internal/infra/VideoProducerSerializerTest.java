...
class VideoProducerSerializerTest {
	...
	void serialize_ShouldThrowRuntimeExceptionOnSerializationError() {
		...
		// Act & Assert
		RuntimeException exception = assertThrows(RuntimeException.class, () -> serializer.serialize(topic, data));
	...
	void serialize_ShouldHandleEmptyData() {
		...
		// Assert
		assertNotNull(result);
		assertTrue(result.length > 0); // O objeto serializado deve ter conteúdo mesmo com
										// dados vazios
	}
