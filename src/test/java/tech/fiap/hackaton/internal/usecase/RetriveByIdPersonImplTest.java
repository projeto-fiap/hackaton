...
class RetriveByIdPersonImplTest {
	...
	void getPersonById_ShouldThrowExceptionWhenPersonNotFound() {
		...
		// Act & Assert
		RuntimeException exception = assertThrows(RuntimeException.class, () -> retriveByIdPerson.getPersonById(id));
	...
	void getPersonById_ShouldMapAllFieldsCorrectly() {
		...
		// Assert
		assertAll(() -> assertEquals(id, result.id()), () -> assertEquals("Maria Souza", result.nome()),
