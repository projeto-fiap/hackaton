...
public class GetUserByVideoHashImpl implements GetUserByVideoHash {
	...
	public PersonWithVideoDTO findUserByVideoHash(String hashNome) {
		...
		return new PersonWithVideoDTO(person.getId(), person.getNome(), person.getCpf(), person.getEmail(),
				video.getId(), video.getNome(), video.getUrl(), video.getStatus().name(), video.getDataCriacao(),
